...
class FakeReconciliationService implements CashoutReconciliationServicePort {
  ...
  public async retryDispatch(cashoutRequestId: string) {
    ...
    return {
      ...
      reconciliationState: "provider_dispatched" as const
    };
  }
}
...
function adminHeaders(role = "support", actor = "support-1"): Record<string, string> {
  return {
    ...
    "x-admin-role": role,
    "x-admin-actor": actor,
    "x-request-id": "req_admin_cashout_test"
  };
}

describe("admin cashout request routes", () => {
  it("returns searchable cashout records for support", async () => {
    const response = await fetch(`${baseUrl}/api/admin/cashout-requests?status=failed_retryable`, {
      headers: adminHeaders()
    });
    const body = await response.json() as ApiEnvelope<{ records: Array<{ cashoutRequestId: string; reconciliationState: string }> }>;
