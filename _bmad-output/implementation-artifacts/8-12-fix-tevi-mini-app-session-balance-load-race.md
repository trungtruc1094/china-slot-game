...
- `js/slotGame.js` — follow-up (from live sandbox logs): `isTerminalSessionReauth` now treats an HTTP **401** from the session / token-exchange endpoint as terminal re-auth. Real `getUserInfo` runtime tokens rejected by the Tevi provider return `POST /api/tevi/token 401` (`PROVIDER_REJECTED` → backend code `TEVI_TOKEN_EXCHANGE_FAILED`); previously the client misclassified that as transient, retried 3×, and showed the generic "Backend unavailable / Reopen" popup instead of the "Sign in to Tevi" re-auth prompt. Keys on the 401 status (not the code) so transient `PROVIDER_UNAVAILABLE`/`PROVIDER_RESPONSE_INVALID` 5xx stay retryable.
- `apps/api/test/unit/server-client.test.ts` — follow-up: +2 tests — 401 token-exchange rejection is terminal (single `startSession`, `reauth-required` + "Tevi" prompt) and a transient 503 with the same code still retries 3× and surfaces `retry`.
- `apps/api/src/domain/tevi-token-service.ts` — diagnostics (to root-cause the live sandbox `PROVIDER_REJECTED` 401): on a provider non-2xx, `logFailure` now also emits token-safe `responseShape` (provider error body's key names only — never values, mirroring the payment client) and `sentTokenClaims` (only the bearer's decoded `exp`/`iat`/`app_id` — never the token or other claims), so a 401 can be triaged as expired-token vs wrong-app vs shape-change from logs. Config was verified correct first: `TEVI_APP_ID=AZX29173` matches the sandbox portal app, `TEVI_API_BASE` is the sandbox host, and the bearer is the real `user_app_token`.
- `apps/api/test/unit/tevi-token-service.test.ts` — +1 test asserting the 401 log carries `responseShape`/`sentTokenClaims` while leaking neither the body value, the raw token, nor non-allow-listed claims.
- **Root cause + fix (reversible, flag-gated):** the live `sentTokenClaims` log proved the rejected `user_app_token` is **fresh and valid** (iat ≈ T-40min, exp = iat+24h, app_id=AZX29173) — so the 401 is **not** expiry. Doc investigation: `GET /api/v1/auth/token` expects a *different* bearer (the Tevi platform session token, which the mini-app SDK cannot provide) and/or a separate Login-Kit scope, so it rejects the `user_app_token`. The fix bypasses the exchange entirely by verifying the `user_app_token` directly against the JWKS — the exact verification the Tevi auth middleware already runs successfully for every payment (existence proof the token is JWKS-verifiable with the needed claims). Gated behind `TEVI_SESSION_AUTH_MODE` (default `exchange` = unchanged; set `direct` on Render to flip, fully reversible):
  - `apps/api/src/config/env.ts` — parse `TEVI_SESSION_AUTH_MODE` into `tokenExchange.sessionAuthMode` (default `exchange`).
  - `apps/api/src/routes/tevi-token.routes.ts` — `direct` branch in `POST /tevi/token`: verify `runtimeToken` via JWKS verifier, bind session from its claims, skip `exchangeRuntimeToken`. Failure → 401 `TEVI_REAUTH_REQUIRED`.
  - `apps/api/src/app.ts`, `apps/api/src/main.ts` — thread `teviSessionAuthMode` from env to the router.
