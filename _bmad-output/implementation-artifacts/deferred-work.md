...
- AC5 manual Tevi sandbox Check Round still unverified — the live money path (land → balance loads → spin debits/credits → deposit +50 reflects without reload) must be run in a real Tevi mini-app sandbox before the story can move to `done`. Capture before/after screenshots + BE logs. By design (no sandbox in dev env).
